...
def update_administration_data():
    log.info(f'Starting update of administration data ... The following data_source models are updated: {source_class_list}')
        ...
        with transaction.atomic(using='moodle'):
            for source_class in source_class_list:
                ...
                if not sink_class_list:
                    log.exception(f'No administration model found for data_source model {source_class}')
                elif len(sink_class_list) > 1:
                    log.exception(f'Ambiguous: More than one administration model for data_source model {source_class}')
                else:
                ...
                if (delete_list):
                    ...
                    log.info(f'app administration, model {source_class.__name__}: deleted rows with id in {delete_list}')
                ...
                if (source_class_instances_list):
                    log.info(f'Processing administration data of model {source_class.__name__}: created/updated rows with id in {source_class_instances_list}')
        log.info(f'Completed update of administration data')